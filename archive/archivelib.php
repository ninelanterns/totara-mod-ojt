...
function ojt_archive_and_add_pdf_file_to_evidence($ojtid, $userid)
{
    ...
    // get observer
    $flaten_topics = call_user_func_array('array_merge', $topics);
    $observerId = reset($flaten_topics['items'])['modifiedby'];
    $currentObserver = $DB->get_record('user', array('id' => $observerId));
    $observerfullname = fullname($currentObserver);
    $observer = "<strong>Observer:</strong> " . $observerfullname . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . userdate(time(), get_string('strftimedatefullshort', 'core_langconfig'));
    ...
    $pdfcontent = ojt_generate_pdf($html, $filename, $title, $subject, $userfullname, $observer);
